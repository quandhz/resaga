...
export function submitFormFailed(pageName, formName, payload, options, error) {
  return {
    type: `${SUBMIT_FAILED}${SEPARATOR}${pageName}${SEPARATOR}${formName}`,
    [PAGE]: pageName,
    formName,
    payload,
    options,
...
export function submitFormSucceed(pageName, formName, payload, options, result) {
  return {
    type: `${SUBMIT_SUCCEED}${SEPARATOR}${pageName}${SEPARATOR}${formName}`,
    [PAGE]: pageName,
    formName,
    payload,
    options,
...
export function doAcknowledge(pageName, formName) {
  return {
    type: `${SUBMIT_ACKED}${SEPARATOR}${pageName}${SEPARATOR}${formName}`,
    [PAGE]: pageName,
    formName,
  };
}
