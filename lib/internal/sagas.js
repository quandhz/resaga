...
export function* handleRequest({ [PAGE]: page, formName, payload, options }) {
  ...
  if (configs[PAGE] === page) {
    ...
    try {
      ...
      yield makeCallback(submitFormSucceed, option.onSubmitSuccess, option.postProcess(result), payload, option.request, formName, options);
    } catch (error) {
      yield makeCallback(submitFormFailed, option.onSubmitError, option.postProcessError(error), payload, option.request, formName, options);
    }
  }
}
