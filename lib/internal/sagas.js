 ...
 * @param page
 */
export function* handleRequest({ name, requestName, payload, options }) {
  const originConfigs = options.configs;
  if (originConfigs.name === name) {
    const configs = analyse(originConfigs, requestName);
    try {
      const result = yield call(configs.requests, payload);
      yield makeCallback(submitFormSucceed, configs.onSubmitSuccess, configs.postProcess(result), payload, configs.name, requestName, options);
    } catch (error) {
      yield makeCallback(submitFormFailed, configs.onSubmitError, configs.postProcessError(error), payload, configs.name, requestName, options);
    }
  }
}
