...
const resaga = (originConfigs) => (Component) => {
  ...
  class Resaga extends React.PureComponent {
    componentWillMount = () => {
      ...
      this.values = helpers.exportValues({ props: this.props, configs }, true);
    };
    ...
    getValue = (...props) =>
      helpers.getValue(this.internalProps, this.props, ...props);
    ...
    analyse = (...props) =>
      helpers.analyseNextProps(...props, this.internalProps);
  ...
  const mapState = (state, props) => {
    ...
    return ({
      [configs.name]: selectors.selectPage(configs.name, configs.manuallySubscribe)(state, props),
      ...subscribe,
    });
  };
