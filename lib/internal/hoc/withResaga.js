...
const resaga = (originConfigs) => (Component) => {
  ...
  class Resaga extends React.PureComponent {
    ...
    render = () => {
      const {
        values, setValue, setValueWithFunc, beforeDispatch, dispatch, acknowledge, cleanup, ...props
      } = this.props;
      const internalProps = {
        Component, configs, values, setValue, setValueWithFunc, beforeDispatch, dispatch, acknowledge, cleanup,
      };
