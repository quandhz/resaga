 ...
 * @returns {function(*=)}
 */
const selectKeyPath = ({ keyPath, notSetValue }, isLoading) =>
  (state, props) => {
    let path = keyPath;
 ...
 * @param selector
 */
const analyseObject = (key, selector, isLoading) => (state, props) => {
  ...
  if (Array.isArray(selector.keyPath) && isKeyPath(selector.keyPath[0])) {
    ...
    keyPath = selector.keyPath.reduce(
      (result, eachSelector) =>
        result.concat(selectKeyPath({ keyPath: eachSelector }, isLoading)),
      []
    );
  } else {
    ...
    if (isLoading) {
      normalise = {
        getter: ({ willLoad, isLoading: loading } = {}) => Boolean(willLoad || loading),
        ...selector,
      };
    }
    keyPath = [selectKeyPath(normalise, isLoading)];
  }
...
const analyse = (key, selector, isLoading) => {
  ...
  switch (type) {
    ...
    case 'function':
      // transform (props) => [...]
      // to { keyPath: (props) => [...] }
      return analyse(key, { keyPath: selector }, isLoading);
    ...
    case 'object':
      if (Array.isArray(selector)) {
        ...
        if (typeof last(selector) === 'function') {
          ...
          return analyse(key, { keyPath, getter: last(selector) }, isLoading);
        }
      ...
      // normalised selector
      return analyseObject(key, selector, isLoading);
...
export const selectIsLoading = (configValue = {}, value = {}) => (state = {}, props = {}) => Object.keys(configValue).reduce(
  (reduction, key) =>
    ({ ...reduction, ...analyse(key, configValue[key], true)(state, props) }),
  value
);
