...
import { SELECTOR_UTILS } from './selectorHelpers';
...
const generateCacheKeys = (originConfigs) => dotProp.set(originConfigs, 'value', (value) => {
  if (typeof value !== 'object') return value;

  return Object.keys(value).reduce((accu, valueKey) => {
    const selector = value[valueKey];

    if (SELECTOR_UTILS.isGetterSelector(selector)) {
      const cacheKey = `${valueKey}_${Math.random()}`;
