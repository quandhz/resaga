...
import selectConfigValue, { selectIsLoading, isGetter } from '../selectConfigValue';
...
describe('selectConfigValue()', () => {
  ...
  describe('selectIsLoading array of strings', () => {
    it('willLoad true', () => {
      const stating = setKeyValue('templates', { content: 'hi ho', willLoad: true });
      const configs = { templates: ['TemplateStore', 'templates'] };
      expect(selectIsLoading(configs)(stating)).toEqual({ templates: true });
    });

    it('isLoading true', () => {
      const stating = setKeyValue('templates', { content: 'hi ho', isLoading: true });
      const configs = { templates: ['TemplateStore', 'templates'] };
      expect(selectIsLoading(configs)(stating)).toEqual({ templates: true });
    });

    it('result', () => {
      const stating = setKeyValue('templates', { content: 'hi ho' });
      const configs = { templates: ['TemplateStore', 'templates'] };
      expect(selectIsLoading(configs)(stating)).toEqual({ templates: false });
    });

    it('acked', () => {
      const stating = setKeyValue('templates', undefined);
      const configs = { templates: ['TemplateStore', 'templates'] };
      expect(selectIsLoading(configs)(stating)).toEqual({ templates: false });
    });

    it('default value', () => {
      expect(selectIsLoading()()).toEqual({});
    });
  });
