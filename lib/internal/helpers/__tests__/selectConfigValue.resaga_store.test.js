...
const initState = fromJS({ TemplateStore: { [VARIABLES]: { templates: {}, id: 1 } } });

const setKeyValue = (key, value) => {
  const templates = initState.getIn(['TemplateStore', VARIABLES]);
  return initState.mergeIn(['TemplateStore'], {
    [VARIABLES]: templates.concat({ [key]: value }),
  });
};
