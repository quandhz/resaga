...
const analyseConfigs = (originConfigs) => {
  ...
  if (configs.page && !configs.name) {
    ...
    errors.ResagaWarning('`CONFIG.page` is renamed to `CONFIG.name`. Please adapt your code. `CONFIG.page` will not be supported in the next minor release.');
  ...
  if (configs.submit && !configs.requests) {
    // TODO: backward compatibility v0.0.x
    errors.ResagaWarning('`CONFIG.submit` is renamed to `CONFIG.requests`. Please adapt your code. `CONFIG.submit` will not be supported in the next minor release.');
...
export const resaga = (originConfigs) => (Component) => {
