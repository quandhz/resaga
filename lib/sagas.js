...
export function* handleRequest({ name, requestName, payload, options }) {
  ...
  if (originConfigs.name === name) {
    ...
    try {
      ...
      yield makeCallback(submitFormSucceed, configs.onSuccess, configs.postProcess(result), payload, configs.name, requestName, options);
    } catch (error) {
      yield makeCallback(submitFormFailed, configs.onError, configs.postProcessError(error), payload, configs.name, requestName, options);
    }
  }
}
