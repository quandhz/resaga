 ...
 * @param page
 */
export function* handleRequest({ data, options, formName, page }) {
  if (options.page === page) {
    const option = analyse(options, formName);
    try {
      const result = yield call(option.submit, data);
      yield makeCallback(submitFormSucceed, option.onSubmitSuccess, option.postProcess(result), data, option.request, formName);
    } catch (error) {
      yield makeCallback(submitFormFailed, option.onSubmitError, option.postProcessError(error), data, option.request, formName);
    }
  }
}
