...
import { RESULT, SERVER_ERROR, SUBMIT_FAILED, SUBMIT_SUCCEED, SUBMIT_SUCCESS, VARIABLES } from '../../internal/constants';
...
describe('utils/hoc/onSubmit/utils/test-helpers', () => {
  ...
  it('test.generate.stateError', () => {
    const expected = reducer(state, { formName: mockString, type: SUBMIT_FAILED });
    expect(test.generate.stateError(mockString)).toEqual(expected);
  });
  it('test.generate.stateSuccess', () => {
    const expected = reducer(state, { formName: mockString, type: SUBMIT_SUCCEED });
