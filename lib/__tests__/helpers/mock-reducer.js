...
const expectBeforeDispatch = (state) => state.set(requestName, {
...
const expectSetValue = (state) => state.merge({
  [VARIABLES]: state.get(VARIABLES, fromJS({})).concat({
    [key]: value,
  }),
});
const expectSetValueNormal = (state) => state.merge(state.concat({
  [key]: value,
}));
const expectSetValueNormalNested = (state) => state.merge(state.concat({
  [key]: {
    deep: {
      nested: value,
    },
