...
const isBrowser = IS_BROWSER ? window : null;
const globalScope = (
...
function getLogPrefix(type, effect) {
  ...
  const isErrorRes = isError ? '✘' : '✓';
  const winnerInd = effect && effect.winner
    ? isErrorRes : '';

  const isErrorStyle = (s) => isError ? ERROR_STYLE : s;
  const style = (s) => isCancel ? CANCEL_STYLE : isErrorStyle(s);
...
function argToString(arg) {
  ...
  return typeof arg === 'function' ? `${arg.name}` : argString;
}
